...
```sh
opencode plugin add github:iqhive/FlowDeck#v2.0.1
```

which adds `"github:iqhive/FlowDeck#v2.0.1"` to the `plugins` array in `opencode.json`. Alternatively, `npx @dv.nghiem/flowdeck install` (`bin/flowdeck.js`) automatically:
