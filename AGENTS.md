...
### Commands
- Install: `./install.sh`
- Build: `bun run bundle` (named `bundle`, not `build`, so npm/pacote does not try to prepare the package when installed from git)
- Test (all): `bun test`
...
- Do NOT modify `dist/` manually — it is generated by `bun run bundle`
